...
const fetchUserInfo = async () => {
  try {
    const res = await request.get({ url: '/admin/users' })
    if (res.data?.list?.length) {
      ...
      userInfo.value = {
        ...
        email: u.email || '',
        avatarUrl: '',
        roleList: Array.isArray(u.roles) ? u.roles : [],
      }
      useUserStore().setUserInfo({ username: u.username, nickname: u.nickname || u.username } as any)
    }
  } catch { /* ignore */ }
}
...
const saveAvatar = async () => {
  try {
    ...
    const file = unref(uploadAvatarRef)?.getFile?.()
    if (file) {
      const fd = new FormData()
      fd.append('file', file)
      const res = await request.upload<{ url: string }>({ url: '/upload', data: fd })
      if (res.data?.url) {
        userInfo.value.avatarUrl = '/api/v1' + res.data.url
    ...
    <ContentWrap title="基本资料" class="flex-[3] ml-20px">
      <ElTabs v-model="activeName">
        <ElTabPane label="基本信息" name="first">
          <EditInfo :user-info="userInfo" @update="fetchUserInfo" />
        </ElTabPane>
